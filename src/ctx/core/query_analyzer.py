...
_INTENT_TERMS = {
    ...
    "refactor": {"cleanup", "consolidate", "modernize", "refactor", "rename", "restructure", "simplify"},
}
...
def analyze_query(query: str, context: dict[str, Any] | None = None) -> QueryAnalysis:
    ...
    return QueryAnalysis(
        ...
        intent=intent,
        task_mode=_infer_task_mode(unique_tokens, intent),
        tokens=tokens,
...
def _infer_task_mode(unique_tokens: tuple[str, ...], intent: str) -> str:
    query_set = set(unique_tokens)
    if intent == "debug":
        return "debug"
    if intent == "explain":
    ...
    if intent == "refactor":
        return "refactor"
    if intent in {"implement", "optimize"}:
        return "build"
    if {"compare", "versus", "vs"}.intersection(query_set):
        return "refactor"
    return "explore"
