...
class ContextDistillation:
    """Deterministic distillation output produced before prompt compilation."""

    rule_extractions: list[str] = field(default_factory=list)
    api_summary: list[str] = field(default_factory=list)
    relationship_map: list[str] = field(default_factory=list)
    priority_order: list[str] = field(default_factory=list)
    compression_notes: list[str] = field(default_factory=list)
    budget_decisions: dict[str, Any] = field(default_factory=dict)
    distillation_confidence: float = 0.0
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "rule_extractions",
            _validate_string_list(self.rule_extractions, "rule_extractions"),
        )
        object.__setattr__(
            self,
            "api_summary",
            _validate_string_list(self.api_summary, "api_summary"),
        )
        object.__setattr__(
            self,
            "relationship_map",
            _validate_string_list(self.relationship_map, "relationship_map"),
        )
        object.__setattr__(
            self,
            "priority_order",
            _validate_string_list(self.priority_order, "priority_order"),
        )
        object.__setattr__(
            self,
            "compression_notes",
            _validate_string_list(self.compression_notes, "compression_notes"),
        )
        object.__setattr__(
            self,
            "evidence_snippets",
            _validate_string_list(self.evidence_snippets, "evidence_snippets"),
        )
        if not isinstance(self.budget_decisions, dict):
            raise TypeError("budget_decisions must be a dictionary.")
        if not isinstance(self.distillation_confidence, (float, int)):
            raise TypeError("distillation_confidence must be numeric.")
        confidence = float(self.distillation_confidence)
        if confidence < 0 or confidence > 1:
            raise ValueError("distillation_confidence must be between 0.0 and 1.0.")
        object.__setattr__(self, "distillation_confidence", confidence)
